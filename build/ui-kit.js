...
function nudgeGainDb(gain, dir, maxGain, dbStep = 1) {
  const db = gain <= 1e-4 ? -60 : 20 * Math.log10(gain);
...
function Knob({ value, min = 0, max = 1, onChange, onBeforeChange, size = 38, label, unit, format, color = "var(--amber)", curve = 1, wheelStep = 0.01 }) {
  ...
  useWheelStep(ref, (dir) => {
    ...
    nv = Math.round(nv / wheelStep) * wheelStep;
    nv = Math.max(min, Math.min(max, nv));
    if (nv === value) return;
    if (onBeforeChange) onBeforeChange();
    onChange(nv);
  ...
  const r = size / 2;
  return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, userSelect: "none" } }, /* @__PURE__ */ React.createElement(
    "div",
    {
      ref,
      onMouseDown: onDown,
    ...
    /* @__PURE__ */ React.createElement("svg", { width: size, height: size, viewBox: `0 0 ${size} ${size}` }, /* @__PURE__ */ React.createElement("circle", { cx: r, cy: r, r: r - 2, fill: "var(--knob-body, #211c16)", stroke: "rgba(0,0,0,.5)", strokeWidth: "1.5" }), /* @__PURE__ */ React.createElement(
      "circle",
      {
        cx: r,
        cy: r,
...
function Meter({ level, height = 120, width = 8, stereo = false }) {
  ...
  for (let i = 0; i < segs; i++) {
    ...
    cells.push(/* @__PURE__ */ React.createElement("div", { key: i, style: { flex: 1, minHeight: 1, background: on ? col : "rgba(0,0,0,.32)", borderRadius: 1, opacity: on ? 1 : 0.85, boxShadow: on ? `0 0 4px ${col}` : "none", transition: "opacity .05s" } }));
  }
...
function Seg({ options, value, onChange, small }) {
  return /* @__PURE__ */ React.createElement("div", { style: { display: "inline-flex", background: "var(--bg)", borderRadius: 8, padding: 2, border: "1px solid var(--line)" } }, options.map((o) => /* @__PURE__ */ React.createElement("button", { key: o.v, onClick: () => onChange(o.v), style: {
    padding: small ? "3px 9px" : "5px 12px",
    fontSize: small ? 11 : 12,
    fontWeight: 600,
    borderRadius: 6,
    background: value === o.v ? "var(--surface3)" : "transparent",
    color: value === o.v ? "var(--cream)" : "var(--muted)"
  } }, o.l)));
}
function fmtTime(s) {
