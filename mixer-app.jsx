...
window.DAW = {
  ...
  // Shift+Mute on a File Tracks channel toggles Mute for EVERY track in that group —
  // stems and bounces alike. The main window is authoritative and echoes the result back
  // via SYNC_STATE; this is only the optimistic local paint, so it must use the same test.
  muteAllFileTracks(next) {
    const on = !!next;
    this.tracks.forEach((t) => { if (isFileGroupTrack(t) && t.params) t.params.mute = on; });
      ...
      <div className="mixer-titlebar" style={{ position: "relative" }}>
        ...
        {(!isMac && window.electronAPI) ? (
          <div className="window-controls" style={{ marginLeft: "auto" }}>
            {/* Minimize removed in v2.6.0: this window is a `parent:` (owned) window, so Windows
                gives it no taskbar button, and `frame: false` leaves nothing to minimise into -
                it collapsed to a stub bar with no way back. See electron/main.js minimizable:false. */}
