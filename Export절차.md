...
마스터 버스(mBus):
  ...
  → mMix
  → Saturation(tanh dry/wet) → Widener(M/S 매트릭스) → Exciter/Enhancer(HPF 배음 wet)   ※ v1.9.6부터
...
### 경로별 차이 (정확한 사실)
- **Saturation / Widener / Exciter / Enhancer** 세 효과는 **v1.9.6부터 Web Audio 폴백의 오프라인
  `renderMix`에도 반영**됩니다(실시간 모니터링·네이티브 엔진 렌더와 동일한 체인을 복제). 세 값이 모두 0이면
  체인은 수학적으로 투명(pass-through)이라 기존 프로젝트 결과는 변하지 않습니다.
  - 단, 체인 종단의 리미터는 경로별로 다릅니다: **실시간/네이티브는 DynamicsCompressor**, **오프라인 렌더는
    제로-지연 소프트 클리퍼**(컴프레서의 ~10ms 지연이 렌더를 밀어내는 것을 피하려는 의도적 정책). 그래서 세
    효과의 음색은 일치하나 최종 리미팅 단의 미세한 다이내믹까지 비트-동일하지는 않습니다.
...
7. 최종 파일을 디스크에 저장한다.
8. 주의: **마스터 Volume은 Export 제외**(모니터 전용). **Saturation/Widener/Exciter는 v1.9.6부터 네이티브·Web Audio 폴백 양쪽 렌더에 모두 반영**(종단 리미터만 경로별로 컴프레서/소프트 클리퍼로 다름).
